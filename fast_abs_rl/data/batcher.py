...
def convert_batch(unk, word2id, batch):
    ...
    sources = convert2id(unk, word2id, sources)
    targets = convert2id(unk, word2id, targets)
    batch = list(zip(sources, targets))
    return batch
...
def convert_batch_copy(unk, word2id, batch):
    ...
    src_exts = convert2id(unk, ext_word2id, sources)
    sources = convert2id(unk, word2id, sources)
    tar_ins = convert2id(unk, word2id, targets)
    targets = convert2id(unk, ext_word2id, targets)
...
def convert_batch_extract_ptr(unk, word2id, batch):
    def convert_one(sample):
        source_sents, extracts = sample
        id_sents = convert2id(unk, word2id, source_sents)
